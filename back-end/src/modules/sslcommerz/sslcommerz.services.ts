...
const paymentInit = async (payload: ISSLPayment) => {
  ...
  try {
    const data = {
      ...
      success_url: `${ENV.SSL_SUCCESS_BACK_END_URL}?transactionId=${payload.transactionId}&amount=${payload.amount}&status=success`,
      fail_url: `${ENV.SSL_FAILED_BACK_END_URL}?transactionId=${payload.transactionId}&amount=${payload.amount}&status=failed`,
      cancel_url: `${ENV.SSL_CANCEL_BACK_END_URL}?transactionId=${payload.transactionId}&amount=${payload.amount}&status=cancel`,
