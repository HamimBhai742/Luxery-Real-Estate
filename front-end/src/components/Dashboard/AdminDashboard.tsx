...
import { FaHome, FaUsers, FaCalendarCheck, FaDollarSign, FaChartLine, FaClock, FaPlus, FaEdit, FaEye, FaMapMarkerAlt } from 'react-icons/fa';
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {stats.map((stat, idx) => (
            <div
              key={idx}
              className="group backdrop-blur-xl bg-white/5 rounded-2xl border border-white/10 p-6 hover:bg-white/10 hover:border-white/20 transition-all duration-300 hover:scale-105 hover:shadow-2xl hover:shadow-purple-500/20"
            >
              <div className="flex items-start justify-between">
                ...
                <div className={`p-4 rounded-xl bg-linear-to-br ${stat.color} group-hover:scale-110 transition-transform duration-300`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 backdrop-blur-xl bg-white/5 rounded-2xl border border-white/10 p-6">
            <div className="flex items-center justify-between mb-6">
              ...
              <select className="bg-white/10 border border-white/20 rounded-lg px-4 py-2 text-white text-sm focus:outline-none focus:ring-2 focus:ring-purple-500">
            ...
            <div className="h-64 flex items-end justify-between gap-2">
              {[40, 70, 45, 80, 60, 90, 75].map((height, idx) => (
                <div key={idx} className="flex-1 flex flex-col items-center gap-2">
                  <div className="w-full bg-linear-to-t from-purple-500 to-blue-500 rounded-t-lg hover:from-purple-400 hover:to-blue-400 transition-all duration-300" style={{ height: `${height}%` }}></div>
                  <span className="text-gray-400 text-xs">Day {idx + 1}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="backdrop-blur-xl bg-white/5 rounded-2xl border border-white/10 p-6">
            ...
            <div className="space-y-4">
              {[
                { action: 'New booking', time: '2 min ago', icon: FaCalendarCheck, color: 'text-green-400' },
                { action: 'Property added', time: '15 min ago', icon: FaHome, color: 'text-blue-400' },
                { action: 'User registered', time: '1 hour ago', icon: FaUsers, color: 'text-purple-400' },
                { action: 'Payment received', time: '2 hours ago', icon: FaDollarSign, color: 'text-orange-400' },
              ].map((activity, idx) => (
                <div key={idx} className="flex items-center gap-4 p-3 rounded-xl bg-white/5 hover:bg-white/10 transition-all duration-300">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {quickActions.map((action, idx) => (
              <Link
                key={idx}
                href={action.href}
                className="group flex items-center gap-4 p-4 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 hover:border-white/20 transition-all duration-300 hover:scale-105"
              >
                <div className={`p-3 rounded-lg bg-linear-to-br ${action.color} group-hover:scale-110 transition-transform duration-300`}>
        ...
        <div className="backdrop-blur-xl bg-white/5 rounded-2xl border border-white/10 p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <Link href="/dashboard/manage-property" className="text-purple-400 hover:text-purple-300 text-sm font-medium transition-colors">
              View All →
            </Link>
          </div>
              ...
              <tbody>
                {recentProperties.map((property, idx) => (
                  <tr key={idx} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                    <td className="py-4 px-4 text-white font-medium">{property.name}</td>
                    <td className="py-4 px-4 text-gray-400 flex items-center gap-2">
                      <FaMapMarkerAlt className="text-purple-400" />
                      {property.location}
                    </td>
                    <td className="py-4 px-4 text-white font-semibold">{property.price}</td>
